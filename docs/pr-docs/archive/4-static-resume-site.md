...
### Tier 11 — Resume content/formatting iteration, print-page-count automation, blog thumbnails, DRY refactor (post-implementation follow-up)

- [x] Bullets switched from `<ul>/<li>` to block `<div>`s with a `-` marker (LinkedIn's rich-text composer collapses `<li>` into single-spaced lines on paste but gives block siblings real paragraph spacing) plus a literal visible-height spacer div between bullets (a zero-height spacer got silently excluded by the browser's rendering-aware copy algorithm — confirmed by testing, not assumed).
- [x] Resume content edits per feedback: added Apple Watch remote-control detail, bolded market research/competitive analysis/GTM strategy and other topic phrases throughout (not just impact metrics), removed an unverified "50+ data scientists" usage claim on the no-more-sql bullet (grant prototype, not awarded, no real usage data), merged the publications bullet into the landmark-study bullet with a Google Scholar link instead of a journal-name list, added Argilla to skills, removed the phone number from resume contact info.
- [x] `entry-head` (job title/date row) no longer wraps the date onto its own line when the title is long — title/org shrinks and wraps internally instead (`flex: 1 1 auto; min-width: 0`), meta stays pinned (`flex: 0 0 auto`). Shortened MSKCC's org field to the abbreviation. Title/org/dates now color-accented (`--link`/`--accent-muted`) and bolder for hierarchy, without increasing font-size; the grey underline stays exclusive to section headers (reusing it at the entry level erased the section/entry hierarchy).
- [x] Fixed the resume 3rd-page print overflow: tightened `@media print` spacing/font-size for headroom, and added `tests/test_resume_page_count.sh` — renders `site/resume.html` to PDF via `npx playwright` and fails `make test`/CI if it ever exceeds 2 pages, so `data/resume.json` growth can't silently break pagination again. (Decided against a print-time JS auto-shrink hack — `beforeprint` timing is unreliable across Safari/Firefox/Chrome and unverifiable in this environment; a loud CI failure beats a clever fix that might silently misbehave.) CI's `test.yaml` now installs Chromium first. Moved the "save as PDF" note to the top of the page and added guidance that the printed date/title/URL/page-number strip is the browser's own print-dialog "Headers and footers" toggle — not something a webpage can suppress.
- [x] Blog listing now shows each post's thumbnail (reusing the image `lib_posts` already auto-detects for SEO), matching the old Quarto listing. Fixed `.cat` (category badge) styling being scoped to `.page-meta .cat` only — on the listing, categories sit under `.post-date` instead, so they rendered with zero spacing/border and ran together as one word; made `.cat` a standalone rule.
- [x] DRY pass across the repo: new `data/site.json` (name, base URL, social/contact links, default OG image) + `scripts/lib_site.py`, replacing `BASE_URL` constants hardcoded separately in `sync_seo.py` and `gen_sitemap.py`, and closing a 3x duplication of GitHub/X/LinkedIn URLs across `templates/nav.html` and `about.html` (visible links row + JSON-LD `sameAs`, the latter two now generated by new `scripts/sync_social.py`). Also fixed an inconsistency this surfaced: `about.html`'s X link was capitalized (`ShaunPorwal`) while everywhere else used lowercase. `data/resume.json`'s contact list and `llms.txt` deliberately stay separate/hand-curated (different curated content, not accidental duplication) — noted why in `docs/development.md`. 16 new tests (`test_lib_site.py`, `test_sync_social.py`, plus fixture updates), suite now 65 tests + the two shell checks.

### Tier 12 — Projects/education layout restructure, languages DRY, wording fix (post-implementation follow-up)

- [x] Selected Projects: link used to sit on its own line below the project name; restructured to the same `entry-head` pattern as experience/education (title left, links right, same row), with a new one-line `description` field carrying the actual impact/what-it-does statement (previously the "sub" line was just stats/links, no explanation of the project itself).
- [x] Education: degree + school name used to be one combined title string joined by an em dash, so a long school name could wrap mid-string and drop underneath the degree, pushing the date/location further down too. Split into separate `degree`/`school` fields — degree + dates/location now share the entry-head row (same fix as experience/projects), school name is its own line below (like the scholarship/honor note), so it can wrap independently without disturbing the dates.
- [x] Found and fixed a real bug surfaced by this: `**{dcurves}**` in the dcurves project description rendered the literal text `{dcurves}` instead of substituting the live count — `lib_resume.py`'s mini-markup parser doesn't support nested tokens (bold content is captured as raw text, not re-parsed). Fixed by not nesting (the count is already bold via its own CSS class, so the `**` wrapper was redundant anyway); documented the no-nesting limitation in `lib_resume.py`'s docstring and `docs/development.md`, added a regression test.
- [x] Languages skills line regrouped by level (English native; Mandarin Chinese + Hindi fluent; Japanese + Spanish conversational) and written so each proficiency word appears once per group rather than once per language — DRY within the existing flat-string skills format, no new schema needed for a single row.
- [x] Corrected the NTU scholarship note: "MOE Full Scholarship" → "MOE Scholarship" (wasn't actually a full scholarship).
- [x] 3 new tests for the restructured projects/education rendering, 1 regression test for the nesting limitation. Suite now 68 tests.
